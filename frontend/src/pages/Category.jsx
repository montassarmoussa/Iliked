...
const CATEGORIES = {
  now_playing: { name: 'Dernières sorties', endpoint: 'movie/now_playing' },
  popular: { name: 'Les plus plébiscités', endpoint: 'movie/popular' },
  top_rated: { name: 'Les mieux notés', endpoint: 'movie/top_rated' },
}
  ...
  const fetchMovies = useCallback(async (p) => {
    ...
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/${category.endpoint}?language=fr-FR&page=${p}`,
    ...
    return (
      <div className="min-h-screen bg-[#0d0d0f]">
        ...
        <div className="pt-24 flex flex-col items-center justify-center">
          <h1 className="text-[#f0f0f0] text-2xl font-bold">Catégorie introuvable</h1>
        </div>
      </div>
    )
  }
    ...
    <PageLoader loading={!dataLoaded || !ready}>
      <div className="min-h-screen bg-[#0d0d0f]">
        ...
        <main className="pt-24 px-6 lg:px-16 pb-12">
          <h1 className="text-[#f0f0f0] text-2xl font-bold mb-8">{category.name}</h1>
